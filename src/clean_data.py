...
def remove_iso_duplicate(df: pd.DataFrame):
    # Removes duplicate iso_a2 column:
    return df.drop(columns=["iso_a2.1"], inplace=True, axis=1)
...
def remove_duplicate_rows(df: pd.DataFrame):
    ...
    return clean_df.drop(df[(duplicates_mask) & ((clean_df.lifeExp < 1.0) | (clean_df.gdpPercap < 1.0))].index) 
